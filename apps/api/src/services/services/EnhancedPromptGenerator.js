...
exports.EnhancedPromptGenerator = exports.PromptTemplateType = void 0;
const AsianCuisineKnowledgeBase_1 = require("../types/AsianCuisineKnowledgeBase");
/**
 * Prompt 模板類型
 */
var PromptTemplateType;
(function (PromptTemplateType) {
    PromptTemplateType["STANDARD"] = "standard";
    PromptTemplateType["ASIAN_CUISINE"] = "asian_cuisine";
    PromptTemplateType["CHINESE"] = "chinese";
    PromptTemplateType["TAIWANESE"] = "taiwanese";
    PromptTemplateType["JAPANESE"] = "japanese";
    PromptTemplateType["KOREAN"] = "korean";
    PromptTemplateType["BEAN_PRODUCTS"] = "bean_products";
    PromptTemplateType["NOODLES"] = "noodles";
    PromptTemplateType["VEGETABLES"] = "vegetables";
    PromptTemplateType["SEAFOOD"] = "seafood";
    PromptTemplateType["MIXED_DISH"] = "mixed_dish";
    PromptTemplateType["COLD_DISH"] = "cold_dish";
    PromptTemplateType["STIR_FRY"] = "stir_fry";
    PromptTemplateType["SOUP"] = "soup";
    PromptTemplateType["INDIGENOUS"] = "indigenous";
    PromptTemplateType["STREET_FOOD"] = "street_food";
...
**Don't just respond with "miso soup", must list soup base and all ingredients!**`;
    ...
    generatePrompt(config = {}) {
        const { imageFeatures, detectedCuisineType, previousAttempts = 0, userFeedback = [], suspectedFoodCategories = [] } = config;
        // 根據配置選擇基礎模板
    ...
    selectBaseTemplate(config) {
        ...
        if (suspectedFoodCategories && suspectedFoodCategories.length > 0) {
            ...
            if (category === AsianCuisineKnowledgeBase_1.FoodCategory.BEAN_PRODUCTS) {
                return this.templates.get(PromptTemplateType.BEAN_PRODUCTS) ||
                    this.templates.get(PromptTemplateType.ASIAN_CUISINE);
            }
            if (category === AsianCuisineKnowledgeBase_1.FoodCategory.NOODLES) {
     ...
     * 添加圖片特徵提示
     */
    addImageFeaturesHint(prompt, features) {
        const hints = [];
     ...
     * 添加料理類型提示
     */
    addCuisineTypeHint(prompt, cuisineType) {
     ...
     * 添加食材類別提示
     */
    addFoodCategoryHints(prompt, categories) {
...
- Pay special attention to check if bottom layer staples (rice, noodles) are identified`;
     ...
     * 添加易混淆食材警告
     */
    addConfusionWarnings(prompt, confusedPairs) {
     ...
     * 添加地方特色背景知識
     */
    addRegionalContext(prompt, region) {
     ...
     * 添加季節性食材提示
     */
    addSeasonalContext(prompt, season) {
        const seasonalFoods = {
            '春': {
                zhTW: '\n\n🌸 春季時令食材：\n- 蔬菜：竹筍、蘆筍、豌豆、韭菜、香椿\n- 水果：草莓、枇杷、桑葚\n- 海鮮：鰻魚、鯖魚',
     ...
     * 基於歷史錯誤優化 Prompt
     */
    addHistoricalErrorContext(prompt, commonErrors) {
...
Respond in JSON format with street food type, features, and toppings.`;
     ...
     * 組合多個增強功能
     */
    enhancePromptWithContext(basePrompt, options) {
     ...
     * @returns 成分識別 prompt
     */
    generateComponentDetectionPrompt(dishName, dishType, region) {
        // 導入成分識別 prompt 函數
        const { generateSoupComponentPrompt, generateFriedRiceComponentPrompt, generateBentoComponentPrompt, generateNoodlesComponentPrompt, generateGenericComponentPrompt } = require('./ComponentDetectionPrompts');
        let basePrompt;
        // 根據料理類型選擇對應的 prompt
        switch (dishType) {
            case ComponentDetection_1.DishType.SOUP:
                basePrompt = generateSoupComponentPrompt(this.language);
                break;
            case ComponentDetection_1.DishType.FRIED_RICE:
                basePrompt = generateFriedRiceComponentPrompt(this.language);
                break;
            case ComponentDetection_1.DishType.BENTO:
                basePrompt = generateBentoComponentPrompt(this.language);
                break;
            case ComponentDetection_1.DishType.NOODLES:
                basePrompt = generateNoodlesComponentPrompt(this.language);
                break;
            case ComponentDetection_1.DishType.STIR_FRY:
            case ComponentDetection_1.DishType.DUMPLING:
            case ComponentDetection_1.DishType.BARBECUE:
            case ComponentDetection_1.DishType.HOT_POT:
            case ComponentDetection_1.DishType.UNKNOWN:
            default:
...
- Portion estimation should consider visual proportion and typical portions`;
     ...
     * @returns 成分精煉 prompt
     */
    generateComponentRefinementPrompt(initialComponents, dishContext) {
...
exports.EnhancedPromptGenerator = EnhancedPromptGenerator;
// 導入 DishType 枚舉以支持成分識別
const ComponentDetection_1 = require("../types/ComponentDetection");
